...
window = Tk()
# window config
window.title('Finance Manager Registration')
window.state('zoomed')
...
#    else:
        ...
        if row!=None:
            ...
            day=date.today()
            cursor.execute('INSERT INTO Employee (emp_id , name , user_name , gender ,date_of_birth ,title= "Finance Manager", email  ,password ,date ) VALUES(?,?,?, ?,?,?, ?,?,)',
                           (empid.get() , name.get() , uname.get() , gender.get() , dob.get()  , email.get()  ,pas.get() , day))  
            conn.commit()
            messagebox.showinfo(title="Client added", message="You have successfully added new employee information.")
            clear()
            fetch_client_info()
...
Label(left_frame, text=" Enter New Finance Manager Information  ", relief=GROOVE, bg='#a9927d', fg="#0a0908",font=("times new roman", 15, 'bold')).place(x=150, y=0)
